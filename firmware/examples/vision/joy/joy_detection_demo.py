...
from src.leds import Color, Leds, Pattern, PrivacyLed
from src.toneplayer import TonePlayer
from src.vision.inference import CameraInference
from src.vision.models import face_detection
from src.vision.streaming.server import StreamingServer
from src.vision.streaming import svg
...
def svg_overlay(faces, frame_size, joy_score):
    ...
    for face in faces:
        ...
        doc.add(
            svg.Text(
                "Joy: %.2f" % face.joy_score, x=x, y=y - 10, fill="red", font_size=30
            )
        )

    doc.add(
        svg.Text(
            "Faces: %d Avg. joy: %.2f" % (len(faces), joy_score),
            x=10,
            y=50,
            fill="red",
            font_size=40,
...
class Photographer(Service):
    ...
    def _draw_face(self, draw, face, scale_x, scale_y):
        ...
        draw_rectangle(
            draw, x, bottom, x + width, text_bottom, 3, fill="white", outline="white"
        )
        draw.text(
            (x + 1 + margin, y + height + 1 + margin),
            text,
            font=self._font,
            fill="black",
        )
    ...
    with contextlib.ExitStack() as stack:
        ...
        if enable_streaming:
            server = stack.enter_context(
                StreamingServer(camera, bitrate=streaming_bitrate, mdns_name=mdns_name)
            )
...
def main():
    ...
    parser.add_argument(
        "--num_frames", "-n", type=int, default=None, help="Number of frames to run for"
    )
    parser.add_argument(
        "--preview_alpha",
        "-pa",
    ...
    parser.add_argument(
        "--image_folder", default="~/Pictures", help="Folder to save captured images"
    )
    parser.add_argument(
        "--blink_on_error",
        default=False,
    ...
    parser.add_argument("--mdns_name", default="", help="Streaming server mDNS name")
