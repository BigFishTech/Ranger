...
Or instead of interacting with the Google Assistant, you can use :mod:`src.cloudspeech`
...
from src.assistant import auth_helpers, device_helpers
from src.board import Led
from src.voice.audio import AudioFormat, Recorder, BytesPlayer
...
AUDIO_FORMAT = AudioFormat(
    sample_rate_hz=AUDIO_SAMPLE_RATE_HZ, num_channels=1, bytes_per_sample=2
)
...
class AssistantServiceClient:
    ...
    def __init__(self, language_code="en-US", volume_percentage=100):
        ...
        self._device_config = embedded_assistant_pb2.DeviceConfig(
            device_model_id=device_model_id, device_id=device_id
        )
    ...
    def _assist(self, recorder, play, deadline):
        ...
        for response in self._assistant.Assist(self._requests(recorder), deadline):
            ...
            if response.audio_out.audio_data:
                ...
                play(
                    _normalize_audio_buffer(
                        response.audio_out.audio_data, self._volume_percentage
                    )
                )
...
class AssistantServiceClientWithLed(AssistantServiceClient):
    ...
    Args:
        board: An instance of :class:`~src.board.Board`.
        language_code: Language expected from the user, in IETF BCP 47 syntax (default is "en-US").
