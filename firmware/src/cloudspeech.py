...
from src.voice.audio import AudioFormat, Recorder

END_OF_SINGLE_UTTERANCE = (
    speech.types.StreamingRecognizeResponse.END_OF_SINGLE_UTTERANCE
)
...
class CloudSpeechClient:
    ...
    def recognize_bytes(self, data, language_code="en-US", hint_phrases=None):
        ...
        streaming_config = speech.types.StreamingRecognitionConfig(
            config=self._make_config(language_code, hint_phrases), single_utterance=True
        )
    ...
    def recognize(self, language_code="en-US", hint_phrases=None):
        ...
        with Recorder() as recorder:
            ...
            requests = (
                speech.types.StreamingRecognizeRequest(audio_content=data)
                for data in chunks
            )
            responses = self._client.streaming_recognize(
